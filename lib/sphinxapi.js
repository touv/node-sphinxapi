...
/* === Build request === */
    ...
    for (var k in values) {
        ...
        req.push(packUInt64(k));

        for (var v in values[k]) {
            if (!values[k].hasOwnProperty(v)) continue;

            if (mva) {
                req.push(pack('>L', [len(values[k][v])]));

                for (var vv in values[k][v]) {
                    if (!values[k][v].hasOwnProperty(vv)) continue;

                    req.push(pack('>L', [values[k][v][vv]]));
                }
            } else {
                req.push(pack('>L', [values[k][v]]));
            }
        }
    }
...
/* Send request */
    ...
    var self = this;
    var request = ConcatBuffer(pack('>2HL', [SphinxClient.SEARCHD_COMMAND_UPDATE, SphinxClient.VER_COMMAND_UPDATE, length]), reqData);
    this._SendRequest(SphinxClient.VER_COMMAND_UPDATE, request, function (err, response) {
        if (err) {
            return fn(err, null);
        }
        ...
        var amount = Number(unpack('>L', response.slice(0, 4)));
        return fn(err, amount);
    });
};
